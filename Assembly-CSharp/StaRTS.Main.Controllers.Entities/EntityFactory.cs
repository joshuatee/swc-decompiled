...
namespace StaRTS.Main.Controllers.Entities
{
	public class EntityFactory : IEventObserver
	{
		...
		{
			Service.Set<EntityFactory>(this);
		...
		{
			BuildingTypeVO buildingType = Service.Get<IDataController>().Get<BuildingTypeVO>(building.Uid);
		...
		public void DestroyEntity(Entity entity, bool destroyView, bool removeSpawnProtection)
		{
			...
			Service.Get<EntityViewManager>().UnloadEntityAsset(entity);
			this.RemoveEntity(entity, removeSpawnProtection);
			if (destroyView)
			{
				Service.Get<EntityViewManager>().DestroyView(entity);
			}
			uint iD = entity.ID;
			Service.Get<EntityController>().RemoveEntity(entity);
			this.entities.Remove(iD);
			Service.Get<EventManager>().SendEvent(EventId.EntityDestroyed, iD);
		}
