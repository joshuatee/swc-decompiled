...
namespace StaRTS.Main.Views.UX.Screens.Leaderboard
{
	public class SquadJoinScreen : AbstractLeaderboardScreen, IEventObserver
	{
		...
		private void LoadFeatureSquads()
		{
			...
			base.AddItemsToGrid<Squad>(Service.Get<LeaderboardController>().FeaturedSquads.List, true, false);
		...
		private void SearchClicked(UXButton btn)
		{
			...
			this.searchBtnOverlay.Enabled = false;
			Service.Get<LeaderboardController>().SearchSquadsByName(this.searchInput.Text, new LeaderboardController.OnUpdateData(this.OnSquadsSearched));
		}

		private void PrepareForNextSearch()
		{
			Service.Get<LeaderboardController>().SearchedSquads.List.Clear();
		}
		...
		private void OnSquadsSearched(bool success)
		{
			...
			List<Squad> list = Service.Get<LeaderboardController>().SearchedSquads.List;
		...
		public override EatResponse OnEvent(EventId id, object cookie)
		{
			...
			case EventId.SquadJoinInviteReceived:
				this.UpdateSquadDataForInvite((SquadInvite)cookie, Service.Get<LeaderboardController>());
				this.UpdateInvitesJewel();
				break;
			case EventId.SquadJoinInvitesReceived:
				this.UpdateSquadDataForInvites((List<SquadInvite>)cookie);
				this.UpdateInvitesJewel();
				Service.Get<EventManager>().UnregisterObserver(this, EventId.SquadJoinInvitesReceived);
				break;
			}
