...
{
	public class TargetIdentifier : ObjectPool<TargetReticle>, IEventObserver
	{
		public const float RETICLE_LIFE_TIME = 1.5f;

		private const float STARSHIP_RETICLE_OFFSET = 0.5f;

		public TargetIdentifier() : base(new ObjectPool<TargetReticle>.CreatePoolObjectDelegate(TargetReticle.CreateTargetReticlePoolObject), null, null, new ObjectPool<TargetReticle>.DeactivatePoolObjectDelegate(TargetReticle.DeactivateTargetReticlePoolObject))
		{
		...
		public EatResponse OnEvent(EventId id, object cookie)
		{
			...
			if (id != EventId.SpecialAttackDeployed)
			{
				if (id == EventId.TroopAcquiredFirstTarget)
				{
					...
					{
						entity = Service.Get<ShooterController>().GetPrimaryTarget(shooterComponent);
					}
					if (entity == null)
					{
						return EatResponse.NotEaten;
					...
					Service.Get<ViewTimerManager>().CreateViewTimer(1.5f, false, new TimerDelegate(this.DeactivateTargetReticle), fromPool);
				}
			}
			else
			{
				...
				Service.Get<ViewTimerManager>().CreateViewTimer(specialAttack.VO.ReticleDuration, false, new TimerDelegate(this.DeactivateTargetReticle), fromPool2);
			}
			return EatResponse.NotEaten;
		}
