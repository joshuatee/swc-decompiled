...
namespace StaRTS.Main.Models.Entities.Components
{
	public class DamageableComponent : ComponentBase
	{
		...
		public DamageableComponent(TransformComponent transform)
		{
			...
			this.board = Service.Get<BoardController>();
		}

		public BoardCell<Entity> FindAttackCell(uint maxRange, int troopWidth, int startX, int startZ)
		{
			return this.FindAttackCell(maxRange, troopWidth, startX, startZ, false);
		}

		public BoardCell<Entity> FindAttackCell(uint maxRange, int troopWidth, int startX, int startZ, bool allowTroopFanning)
		{
			Service.Get<BoardController>().Board.RefreshClearanceMap();
			bool flag = false;
			BoardCell<Entity> boardCellWithMinimumWeight = this.GetBoardCellWithMinimumWeight(maxRange, startX, startZ, troopWidth, out flag, allowTroopFanning);
			...
			{
				Service.Get<StaRTSLogger>().WarnFormat("Failed to find safe attack spot for {0}", new object[]
				{
		...
		private BoardCell<Entity> FindNextSafeSpot(int troopWidth, out BoardCell<Entity> pathCell, ref int lastIndex)
		{
			int num = lastIndex;
			while (true)
			{
		...
		public BoardCell<Entity> FindASafeSpawnSpot(int troopWidth, out BoardCell<Entity> pathCell)
		{
			return this.FindNextSafeSpot(troopWidth, out pathCell, ref this.lastSpawnIndex);
		}

		public BoardCell<Entity> FindNextPatrolPoint(int troopWidth, ref int locIndex)
		{
			this.AdvanceQuadrant(ref locIndex);
			BoardCell<Entity> result;
		...
		private BoardCell<Entity> GetBoardCellWithMinimumWeight(uint maxRange, int startX, int startZ, int troopWidth, out bool isIdealCell, bool allowTroopFanning)
		{
			...
			BoardCell<Entity> boardCellFromIndex = this.GetBoardCellFromIndex(i, num3, troopWidth);
			...
			for (int j = troopWidth; j >= 0; j--)
			{
				for (i = 0; i <= num; i++)
				{
					...
					else
					{
						...
						BoardCell<Entity> boardCellFromIndex2 = this.GetBoardCellFromIndex(i, num5, troopWidth);
					...
					for (int k = 1; k <= numNeightborsToEvaluate; k++)
					{
						...
						{
							BoardCell<Entity> boardCellFromIndex3 = this.GetBoardCellFromIndex(i, num6, troopWidth);
						...
						{
							BoardCell<Entity> boardCellFromIndex3 = this.GetBoardCellFromIndex(i, num7, troopWidth);
							if (boardCellFromIndex3.Clearance >= j)
							{
		...
		private BoardCell<Entity> GetBoardCellFromIndex(int rangeIndex, int positionIndex, int troopWidth)
		{
		...
		private BoardCell<Entity> AdvanceIndex(int troopWidth, ref int lastIndex)
		{
			int num = 0;
			...
			Service.Get<StaRTSLogger>().ErrorFormat("Failed to AdvanceIndex: lastIndex {0}", new object[]
			{
				this.lastSpawnIndex
			});
			return null;
		...
		private void AdvanceQuadrant(ref int lastIndex)
		{
			...
			Service.Get<StaRTSLogger>().ErrorFormat("Failed to AdvanceQuadrant: lastIndex {0}", new object[]
			{
				lastIndex
			});
		}
		...
		protected internal DamageableComponent(UIntPtr dummy) : base(dummy)
		{
		}

		public unsafe static long $Invoke0(long instance, long* args)
		{
			return GCHandledObjects.ObjectToGCHandle(((DamageableComponent)GCHandledObjects.GCHandleToObject(instance)).CalculateAttackPosition(*(int*)args, *(int*)(args + 1), *(int*)(args + 2), *(int*)(args + 3), *(int*)(args + 4)));
		}

		public unsafe static long $Invoke1(long instance, long* args)
		{
			return GCHandledObjects.ObjectToGCHandle(((DamageableComponent)GCHandledObjects.GCHandleToObject(instance)).GetBoardCellFromIndex(*(int*)args, *(int*)(args + 1), *(int*)(args + 2)));
		}

		public unsafe static long $Invoke2(long instance, long* args)
		{
			return GCHandledObjects.ObjectToGCHandle(((DamageableComponent)GCHandledObjects.GCHandleToObject(instance)).GetLastSpawnIndex());
		}

		public unsafe static long $Invoke3(long instance, long* args)
		{
			return GCHandledObjects.ObjectToGCHandle(((DamageableComponent)GCHandledObjects.GCHandleToObject(instance)).GetNumNeightborsToEvaluate(*(int*)args));
		}

		public unsafe static long $Invoke4(long instance, long* args)
		{
			((DamageableComponent)GCHandledObjects.GCHandleToObject(instance)).Init();
			return -1L;
		}
	}
}
