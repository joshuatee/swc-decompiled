...
namespace StaRTS.Main.Models.Entities.Components
{
	public class GeneratorViewComponent : ComponentBase
	{
		...
		public void ShowAmountCollectedText(int amount, CurrencyType currencyType)
		{
			...
			this.textLabel.Text = Service.Get<Lang>().Get("PLUS", new object[]
			{
				amount
			});
			switch (currencyType)
			...
			this.timerDt = 0f;
			this.textTimerId = Service.Get<ViewTimerManager>().CreateViewTimer(0.01f, true, new TimerDelegate(this.OnTextTimer), null);
		}
		...
		private void KillTextTimer()
		{
			...
			{
				Service.Get<ViewTimerManager>().KillViewTimer(this.textTimerId);
				this.textTimerId = 0u;
			}
		}
