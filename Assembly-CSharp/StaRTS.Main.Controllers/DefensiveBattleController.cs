...
namespace StaRTS.Main.Controllers
{
	public class DefensiveBattleController : IEventObserver
	{
		...
		public void StartDefenseMission(CampaignMissionVO mission)
		{
			...
			this.troopController = Service.Get<TroopController>();
			...
			{
				Service.Get<RaidDefenseController>().OnStartRaidDefenseMission();
			}
			...
			TrapViewController trapViewController = Service.Get<TrapViewController>();
		...
		public void EndEncounter()
		{
			...
			{
				Service.Get<RaidDefenseController>().OnEndRaidDefenseMission(this.waves[this.currentWaveIndex].Encounter.Uid);
			}
			...
			Service.Get<CurrentPlayer>().DamagedBuildings = Service.Get<BattleController>().GetBuildingDamageMap();
		}
		...
		private void MoveCameraToAction()
		{
			...
			if (mostReleventCameraEvents != null && mostReleventCameraEvents.Count > 0)
			{
				...
				Vector3 vector = Service.Get<CameraManager>().MainCamera.Camera.WorldToViewportPoint(zero);
		...
		private void DeployTroopGroup(int index, TroopTypeVO troop, DefenseTroopGroup group)
		{
			...
			if (entity != null)
			{
				...
				Service.Get<BattleController>().OnTroopDeployed(troop.Uid, TeamType.Attacker, nearestValidBoardPosition);
				Service.Get<EventManager>().SendEvent(EventId.TroopDeployed, entity);
			}
		}
		...
		private IntPosition GetNearestValidBoardPosition(int locX, int locZ)
		{
			...
			BoardCell<Entity> cellAt = board.GetCellAt(locX, locZ);
