...
namespace StaRTS.Main.Controllers
{
	public class SocialDataController : ISocialDataController
	{
		...
		private void OnGetFriendsData(IGraphResult result)
		{
			...
			{
				Service.Get<Logger>().ErrorFormat("Error fetching FB friends data: {0}", new object[]
				{
					result.Error
				});
			}
		...
		private void OnFacebookInviteFriends(IAppRequestResult result)
		{
			...
			if (jsonFromFBResult != null)
			{
				...
				{
					list = (List<object>)jsonFromFBResult["to"];
				}
				if (list != null)
				{
					int count = list.Count;
		...
		{
			SocialDataController.<DownloadProfileImageCoroutine>c__Iterator15 <DownloadProfileImageCoroutine>c__Iterator = new SocialDataController.<DownloadProfileImageCoroutine>c__Iterator15();
			<DownloadProfileImageCoroutine>c__Iterator.url = url;
			<DownloadProfileImageCoroutine>c__Iterator.callback = callback;
			<DownloadProfileImageCoroutine>c__Iterator.cookie = cookie;
			<DownloadProfileImageCoroutine>c__Iterator.<$>url = url;
			<DownloadProfileImageCoroutine>c__Iterator.<$>callback = callback;
			<DownloadProfileImageCoroutine>c__Iterator.<$>cookie = cookie;
			return <DownloadProfileImageCoroutine>c__Iterator;
		}
