...
namespace StaRTS.Main.Controllers
{
	public class RewardManager
	{
		...
		{
			Service.Set<RewardManager>(this);
		...
		public string GetRewardString(string rewardUid)
		{
			...
			RewardVO vo = this.sdc.Get<RewardVO>(rewardUid);
			return this.GetRewardString(vo, Service.Get<Lang>().Get("SupplyRewardFormat", new object[0]));
		}
		...
		private string GetRewardStringInternal(RewardVO vo, string formatString, bool withLevel)
		{
			...
			if (vo.BuildingUnlocks != null)
			{
				...
				while (i < num)
				{
					...
					bool unlocked = Service.Get<CurrentPlayer>().UnlockedLevels.Buildings.Has(buildingTypeVO);
			...
			if (vo.TroopUnlocks != null)
			{
				...
				while (i < num)
				{
					...
					bool unlocked = Service.Get<CurrentPlayer>().UnlockedLevels.Troops.Has(troopTypeVO);
			...
			if (vo.HeroUnlocks != null)
			{
				...
				while (i < num)
				{
					...
					bool unlocked = Service.Get<CurrentPlayer>().UnlockedLevels.Troops.Has(troopTypeVO2);
			...
			if (vo.SpecialAttackUnlocks != null)
			{
				...
				while (i < num)
				{
					...
					bool unlocked = Service.Get<CurrentPlayer>().UnlockedLevels.Starships.Has(specialAttackTypeVO);
