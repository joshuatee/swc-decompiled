	...
	public class TransmissionsHolonetPopupView
	{
		...
		private void OnNextTransmission(UXButton btn)
		{
			...
			if (this.maxIndex > 1)
			{
				...
				Service.Get<HolonetController>().UpdateIncomingTransmission(this.currentIndex);
			}
		}
		...
		{
			this.CloseTransmission();
			Service.Get<HolonetController>().HandleCallToActionButton("battle", this.currentTransmission.Btn1Data);
