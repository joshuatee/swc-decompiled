...
namespace StaRTS.Main.Views.UX.Screens.ScreenHelpers.Holonet
{
	public class TransmissionsHolonetTab : AbstractHolonetTab
	{
		...
		public TransmissionsHolonetTab(HolonetScreen holoScreen, HolonetControllerType holonetControllerType) : base(holoScreen, holonetControllerType)
		{
			...
			List<TransmissionVO> transmissions = Service.Get<HolonetController>().TransmissionsController.Transmissions;
			...
			for (int i = 0; i < count; i++)
			{
				...
				case TransmissionType.DailyCrateReward:
				{
					...
					CrateVO crateVO = Service.Get<IDataController>().Get<CrateVO>(transmissionVO.CrateId);
		...
		protected override void FeaturedButton1Clicked(UXButton button)
		{
			...
			Service.Get<HolonetController>().HandleCallToActionButton(transmissionVO.Btn1Action, transmissionVO.Btn1Data, transmissionVO.Uid);
		...
		{
			TransmissionVO transmissionVO = (TransmissionVO)button.Tag;
			Service.Get<HolonetController>().HandleCallToActionButton(transmissionVO.Btn2Action, transmissionVO.Btn2Data, transmissionVO.Uid);
