...
namespace StaRTS.Main.Models.ValueObjects
{
	public class TroopTypeVO : IAssetVO, IAudioVO, IDeployableVO, IGeometryVO, IHealthVO, IShooterVO, ISpeedVO, ITroopDeployableVO, ITroopShooterVO, IUnlockableVO, IUpgradeableVO, IValueObject
	{
		...
		public void ReadRow(Row row)
		{
			...
			{
				Service.Get<Logger>().Warn(this.Uid + " must have size 1.  Please fix CMS.");
				this.Size = 1;
			}
			...
			{
				Service.Get<Logger>().WarnFormat("Troop {0} can target something it can't damage", new object[]
				{
					this.Uid
				});
			}
			string text = row.TryGetString(TroopTypeVO.COLUMN_deathProjectile);
			this.DeathProjectileType = ((!string.IsNullOrEmpty(text)) ? dataController.Get<ProjectileTypeVO>(text) : null);
			...
			{
				Service.Get<Logger>().ErrorFormat("Missing rotation speed for troopTypeVO {0}", new object[]
				{
					this.Uid
				});
			}
