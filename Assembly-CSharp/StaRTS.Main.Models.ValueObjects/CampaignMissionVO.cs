	...
	public class CampaignMissionVO : IValueObject
	{
		...
		public void ReadRow(Row row)
		{
			...
			if (!string.IsNullOrEmpty(this.CampaignUid))
			{
				...
				{
					Service.Get<Logger>().ErrorFormat("CampaignMissionVO {0} that references a CampaignVO Uid {1} that doesn't exist", new object[]
					{
						this.Uid,
						this.CampaignUid
					});
		...
		private int ParseCampaignPoints(string raw)
		{
			...
			if (num != 1f)
			{
				Service.Get<Logger>().WarnFormat("The campaign point distribution for mission {0} does not add up to 100: {1}", new object[]
				{
					this.Uid,
					raw
				});
