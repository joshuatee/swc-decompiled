...
{
	public class LimitedEditionItemVO : IGeometryVO, IValueObject
	{
		...
		public void ReadRow(Row row)
		{
			...
			if (!string.IsNullOrEmpty(text))
			{
				...
				{
					this.StartTime = 0;
					Service.Get<Logger>().Warn("LimitedEditionItemVO:: LEI CMS Start Date Format Error: " + this.Uid);
				}
			}
			else
			{
				this.StartTime = 0;
				Service.Get<Logger>().Warn("LimitedEditionItemVO:: LEI CMS Start Date Not Specified For: " + this.Uid);
			}
			...
			if (!string.IsNullOrEmpty(text2))
			{
				...
				catch (Exception)
				{
					...
					Service.Get<Logger>().Warn("LimitedEditionItemVO:: LEI CMS End Date Format Error: " + this.Uid);
				}
			}
			else
			{
