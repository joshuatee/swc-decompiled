	...
	public class BattleLogScreen : ClosableScreen, IEventObserver
	{
		...
		private void OnTabCheckboxSelected(UXCheckbox checkbox, bool selected)
		{
			...
			if (battleLogTab != this.curTab)
			{
				...
				Service.Get<EventManager>().SendEvent(EventId.BattleLogScreenTabSelected, battleLogTab);
			}
		}
		...
		private void AddExpendedItem(UXGrid expendedGrid, string entryItemUid, string troopUid, IUpgradeableVO troop, int count, int unitQuality, BattleEntry battleEntry)
		{
			...
			Service.Get<EventManager>().SendEvent(EventId.ButtonCreated, new GeometryTag(troop, projectorConfig, battleEntry));
		...
		private void OnReplayButtonClicked(UXButton button)
		{
			...
			Service.Get<PvpManager>().ReplayBattle(battleEntry.RecordID, battleEntry.Defender, null);
			Service.Get<EventManager>().SendEvent(EventId.BattleLogScreenReplayButtonClicked, null);
		}
		...
		{
			Service.Get<EventManager>().RegisterObserver(this, EventId.PvpRevengeOpponentNotFound);
			Service.Get<EventManager>().SendEvent(EventId.BattleLogScreenRevengeButtonClicked, null);
			Service.Get<PvpManager>().StartRevenge(entry.Attacker.PlayerId);
		}

		private void OnShareButtonClicked(UXButton button)
		{
			BattleEntry battleEntry = button.Tag as BattleEntry;
			Service.Get<EventManager>().SendEvent(EventId.BattleLogScreenShareButtonClicked, null);
			Service.Get<ScreenController>().AddScreen(new BattleReplayShareScreen(battleEntry));
		}
		...
		public override EatResponse OnEvent(EventId id, object cookie)
		{
			...
			{
				Service.Get<EventManager>().UnregisterObserver(this, EventId.PvpRevengeOpponentNotFound);
				this.ToggleRevengeButtons(true);
			}
