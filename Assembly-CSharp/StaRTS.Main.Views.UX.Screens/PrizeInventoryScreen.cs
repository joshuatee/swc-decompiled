...
namespace StaRTS.Main.Views.UX.Screens
{
	public class PrizeInventoryScreen : ClosableScreen, IEventObserver, IViewClockTimeObserver
	{
		...
		{
			this.cp = Service.Get<CurrentPlayer>();
		}
		...
		protected override void OnScreenLoaded()
		{
			...
			this.InitTabJewels();
			this.InitButtons();
			this.prefs = Service.Get<SharedPlayerPrefs>();
		...
		private void SetupCurTabElements()
		{
			...
			Service.Get<EventManager>().UnregisterObserver(this, EventId.InventoryCrateOpenedAndGranted);
			...
			case InventoryTab.Crate:
				...
				Service.Get<EventManager>().RegisterObserver(this, EventId.InventoryCrateOpenedAndGranted);
		...
		private void AddGoToStoreItem(List<UXElement> list)
		{
			...
			IGeometryVO vo = Service.Get<IDataController>().Get<CrateVO>(cRATE_INVENTORY_TO_STORE_LINK_CRATE_ASSET);
		...
		private UXElement CreateTileWithPrizeInfo(string id, int count, PrizeType prizeType)
		{
			...
			case PrizeType.Crate:
			{
				...
				geometryVO = Service.Get<IDataController>().Get<CrateVO>(crateData.CrateId);
				break;
			}
			}
		...
		private void SetupGridItemQuality(string itemUID, int quality)
		{
			...
			if (quality == 0)
			{
				UXUtils.HideAllQualityCards(this.itemGrid, itemUID, "QualityCardQ{0}");
		...
		private UXElement CreateInventoryGridItem(string id, PrizeType prizeType, string title, string description, int count, IGeometryVO vo, int quality)
		{
			...
			else if (prizeType == PrizeType.Crate)
			{
				...
				{
					PlanetVO planetVO = Service.Get<IDataController>().Get<PlanetVO>(crateData.PlanetId);
		...
		private void OnInfoButtonClicked(UXButton button)
		{
			...
			if (finalUnitFromPrize != null)
			{
				...
				Service.Get<ScreenController>().AddScreen(new DeployableInfoScreen(troopUpgradeTag, null, showUpgradeControls, availableTroopResearchLab));
			}
			else
			{
