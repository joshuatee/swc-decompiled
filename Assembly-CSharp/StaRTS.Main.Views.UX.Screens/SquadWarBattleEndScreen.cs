...
namespace StaRTS.Main.Views.UX.Screens
{
	public class SquadWarBattleEndScreen : AbstractBattleEndScreen
	{
		...
		public override void OnDestroyElement()
		{
			...
			Service.Get<AudioManager>().ClearAudioRepeatTimers();
		...
		protected override void OnAllStarsComplete(uint id, object cookie)
		{
			...
			{
				Service.Get<AudioManager>().PlayAudioDelayed("sfx_stinger_factory_outpost_captured", 1f);
			}
			else if (text == "BaseChangeOwner")
			{
				Service.Get<AudioManager>().PlayAudioDelayed("sfx_stinger_factory_outpost_captured", 2f);
			}
