...
namespace StaRTS.Main.Views.UX.Screens
{
	public class BuildingInfoScreen : SelectedBuildingScreen, IViewClockTimeObserver, IEventObserver
	{
		...
		protected override void SetSelectedBuilding(Entity newSelectedBuilding)
		{
			...
			this.reqMet = Service.Get<UnlockController>().IsUnlocked(this.nextBuildingInfo, 1, out this.reqBuildingInfo);
		}
		...
		protected void AddTroopItem(IUpgradeableVO troop, int troopCount, string tooltipText)
		{
			...
			Service.Get<EventManager>().SendEvent(EventId.ButtonCreated, new GeometryTag(troop, config, Service.Get<CurrentPlayer>().ActiveArmory));
		...
		private void OnTroopItemClicked(UXButton button)
		{
			...
			Service.Get<ScreenController>().AddScreen(new DeployableInfoScreen(troopUpgradeTag, this.troopList, showUpgradeControls, availableTroopResearchLab));
		}
		...
		protected void ConfirmInstantUpgrade(object result, object cookie)
		{
			...
			Service.Get<DMOAnalyticsController>().LogInAppCurrencyAction(currencyAmount, itemType, uid, itemCount, type, subType);
			...
			try
			{
				...
				supportController.StartBuildingUpgrade(this.nextBuildingInfo, this.selectedBuilding, true);
				supportController.BuyOutCurrentBuildingContract(this.selectedBuilding, false);
			}
			finally
			{
