...
namespace StaRTS.Main.Views.UX.Screens
{
	public class TroopUpgradeScreen : SelectedBuildingScreen, IViewFrameTimeObserver
	{
		...
		private const string TROOP_CARD_Q_PREFIX = "CardQ{0}";
		...
		public TroopUpgradeScreen(Entity trainingBuilding) : base("gui_upgrade_troops", trainingBuilding)
		{
			...
			this.troopUpgradeCatalog = Service.Get<TroopUpgradeCatalog>();
			this.starshipUpgradeCatalog = Service.Get<StarshipUpgradeCatalog>();
		...
		protected override void OnScreenLoaded()
		{
			...
			Service.Get<DeployableShardUnlockController>().AllowResearchBuildingBadging = false;
		...
		private void PopulateEquipmentGrid()
		{
			...
			this.FindAllUpgradableEquipment(player, equipmentUpgradeCatalog);
			...
			while (i < count)
			{
				...
				if (equipmentTabHelper.IsEquipmentValidForTab(equipmentVO, currentTab))
				{
					...
					UXElement item = this.FetchEquipmentCardGridItem(player, equipmentVO, equipmentUpgradeCatalog);
					list.Add(item);
				}
				i++;
			}
		...
		private void OnEquipmentButtonClicked(UXButton button)
		{
			...
			Service.Get<ScreenController>().AddScreen(new EquipmentInfoScreen(selectedEquipment, this.equipmentToDisplay, this.selectedBuilding, true));
		}
		...
		private void UpdateContractTimers()
		{
			...
			if (remainingTimeForView > 0)
			{
				...
				if (this.activeContract.DeliveryType == DeliveryType.UpgradeEquipment)
				{
					...
					EquipmentVO prevLevel = Service.Get<EquipmentUpgradeCatalog>().GetPrevLevel(vo);
					EquipmentTabHelper equipmentTabHelper = this.filterHelper as EquipmentTabHelper;
		...
		private void AddUpgradeableItemToTroopGrid(IDeployableVO nextTroop, IDeployableVO currentTroop, int sortOrder)
		{
			...
			if (flag3)
			{
				...
				UXUtils.SetCardQuality(this, this.troopGrid, uid, quality, "CardQ{0}");
				UXUtils.SetupFragmentIconSprite(subElement4, quality);
				IDeployableVO troopType = currentTroop;
				if (Service.Get<UnlockController>().IsMinLevelUnlocked(currentTroop))
				{
					troopType = nextTroop;
				}
		...
		private void SetupTroopShardProgressBar(string itemUid, IDeployableVO troopType, bool isMaxLevel)
		{
			...
			int shardAmount = Service.Get<DeployableShardUnlockController>().GetShardAmount(troopType.UpgradeShardUid);
		...
		private void OnTroopCardClicked(UXButton button)
		{
			...
			Service.Get<ScreenController>().AddScreen(new DeployableInfoScreen(this.GetPrevLevel(tag), list, true, this.selectedBuilding));
		}
		...
		private TroopUpgradeTag GetPrevLevel(TroopUpgradeTag tag)
		{
			...
			{
				deployableVO = Service.Get<StarshipUpgradeCatalog>().GetPrevLevel(tag.Troop as SpecialAttackTypeVO);
			}
			else if (tag.Troop is TroopTypeVO)
			{
				deployableVO = Service.Get<TroopUpgradeCatalog>().GetPrevLevel(tag.Troop as TroopTypeVO);
			}
			string requirementText = null;
			string shortRequirementText = null;
			bool reqMet = Service.Get<UnlockController>().CanDeployableBeUpgraded(deployableVO, tag.Troop, out requirementText, out shortRequirementText);
		...
		public override void OnDestroyElement()
		{
			...
			Service.Get<EventManager>().UnregisterObserver(this, EventId.InventoryUnlockUpdated);
			this.DisableTimers();
