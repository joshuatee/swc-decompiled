...
namespace StaRTS.Main.Views.UX.Screens
{
	public class SquadWarScreen : ClosableScreen, IEventObserver, IViewClockTimeObserver
	{
		...
		public void CloseSquadWarScreen(TransitionCompleteDelegate callback)
		{
			...
			Service.Get<SquadController>().WarManager.ExitWarBoardMode(callback);
			this.Close(null);
		}
		...
		protected override void OnScreenLoaded()
		{
			...
			this.InitBuffBaseUI();
			this.UpdateWarElements();
			switch (Service.Get<SquadController>().WarManager.GetCurrentStatus())
			{
		...
		protected override void InitButtons()
		{
			...
			base.GetElement<UXButton>("BtnInfo").OnClicked = new UXButtonClickedDelegate(Service.Get<SquadController>().WarManager.ShowInfoScreen);
		}
		...
		private void InitActionAndCooldownPhase()
		{
			...
			SquadWarSquadType currentDisplaySquad = Service.Get<WarBoardViewController>().GetCurrentDisplaySquad();
		...
		private void SetBuffBaseUI(SquadWarStatusType status)
		{
			...
			{
				SquadWarSquadData squadData = Service.Get<SquadController>().WarManager.GetSquadData(SquadWarSquadType.PLAYER_SQUAD);
				SquadWarSquadData squadData2 = Service.Get<SquadController>().WarManager.GetSquadData(SquadWarSquadType.OPPONENT_SQUAD);
		...
		private void ShowCooldownPhase()
		{
			...
			if (SquadUtils.CanStartMatchmakingPrep(squadController, blc))
			{
		...
		{
			TimerAlertScreen highestLevelScreen = Service.Get<ScreenController>().GetHighestLevelScreen<TimerAlertScreen>();
		...
		{
			Service.Get<SquadController>().WarManager.ShowWarEndedScreen();
		}
		...
		private void CreateBuffBaseElement(UXGrid grid, WarBuffVO warBuffVO, string renderSpriteName, SquadWarBuffBaseData buffBaseData, string buffIconName, string infoLableName, string spriteLockName, SquadWarSquadData squad)
		{
			...
			PlanetVO vo = Service.Get<IDataController>().Get<PlanetVO>(warBuffVO.PlanetId);
			...
			subElement3.Visible = !Service.Get<CurrentPlayer>().IsPlanetUnlocked(warBuffVO.PlanetId);
		...
		private void OnBuffBaseClicked(UXCheckbox checkbox, bool selected)
		{
			...
			Service.Get<EventManager>().SendEvent(EventId.WarBoardBuffBaseBuildingSelected, checkbox);
		}
		...
		{
			Service.Get<SquadController>().ShowTroopRequestScreen(null, true);
		}
		...
		private void OnSquadMemberItemButtonClicked(UXButton button)
		{
			...
			Service.Get<WarBoardBuildingController>().ShowWarBuildings(participantSquad, false);
			Service.Get<WarBoardViewController>().SelectAndCenterOn(squadMemberId);
		}

		private void OnFilterPlayerCheckboxSelected(UXCheckbox checkbox, bool selected)
		{
			Service.Get<WarBoardBuildingController>().ShowWarBuildings(SquadWarSquadType.PLAYER_SQUAD, true);
		}

		private void OnFilterOpponentCheckboxSelected(UXCheckbox checkbox, bool selected)
		{
			Service.Get<WarBoardBuildingController>().ShowWarBuildings(SquadWarSquadType.OPPONENT_SQUAD, true);
		}
