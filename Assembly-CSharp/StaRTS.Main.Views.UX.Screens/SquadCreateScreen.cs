	...
	public class SquadCreateScreen : ClosableScreen
	{
		...
		private void OnCreateComplete(bool success, object cookie)
		{
			...
			if (success)
			{
				...
				Service.Get<UXController>().MiscElementsManager.ShowPlayerInstructions(this.lang.Get("CREATE_SQUAD_CONFIRMATION", new object[]
				{
					text
				}), 1f, 5f);
		...
		{
			Service.Get<EventManager>().SendEvent(EventId.SquadCredits, null);
			string text = this.inputName.Text;
		...
		{
			ProfanityController profanityController = Service.Get<ProfanityController>();
