...
namespace StaRTS.Main.Views.UX.Screens
{
	public class RaidInfoScreen : ClosableScreen, IViewClockTimeObserver
	{
		...
		private void SetupRaidData()
		{
			...
			this.raidPool = Service.Get<IDataController>().Get<RaidMissionPoolVO>(currentPlayer.CurrentRaidPoolId);
			this.raidMission = currentPlayer.CurrentRaid;
			this.raidAvailable = Service.Get<RaidDefenseController>().IsRaidAvailable();
		}
		...
		private void SetupRaidRewards()
		{
			...
			CrateVO crateVO = Service.Get<IDataController>().Get<CrateVO>(this.raidPool.Crate1StarRewardId);
			CrateVO crateVO2 = Service.Get<IDataController>().Get<CrateVO>(this.raidPool.Crate2StarRewardId);
			CrateVO crateVO3 = Service.Get<IDataController>().Get<CrateVO>(this.raidPool.Crate3StarRewardId);
		...
		{
			if (this.raidAvailable != Service.Get<RaidDefenseController>().IsRaidAvailable())
			{
				this.RefreshRaidView();
			}
			this.UpdateRaidTimer();
		...
		{
			Service.Get<EventManager>().SendEvent(EventId.RaidNotifyRequest, null);
			this.Close(null);
		}
