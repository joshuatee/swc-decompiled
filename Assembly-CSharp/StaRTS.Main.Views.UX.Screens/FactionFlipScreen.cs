...
namespace StaRTS.Main.Views.UX.Screens
{
	public class FactionFlipScreen : ClosableScreen
	{
		...
		protected override void InitButtons()
		{
			...
			this.swapButton.Enabled = ((!this.locked && Service.Get<CurrentPlayer>().NumIdentities == 1) || this.oppositePlayerInfo != null);
		...
		{
			string pref = Service.Get<ServerPlayerPrefs>().GetPref(ServerPref.FactionFlippingSkipConfirmation);
			if (pref == "0")
			{
		...
		{
			Service.Get<EventManager>().SendEvent(EventId.UIFactionFlipAction, "close");
