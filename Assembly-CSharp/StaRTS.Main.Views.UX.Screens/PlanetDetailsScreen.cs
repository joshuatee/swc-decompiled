...
namespace StaRTS.Main.Views.UX.Screens
{
	public class PlanetDetailsScreen : ClosableScreen
	{
		...
		public void UpdateCurrentPlanet(Planet planet)
		{
			...
			this.planetLockedInfo.Visible = !Service.Get<CurrentPlayer>().IsPlanetUnlocked(this.viewingPlanetVO.Uid);
		...
		public void ShowObjectivesUI()
		{
			...
			if (base.Root != null)
			{
				...
				this.animDelayTimerId = Service.Get<ViewTimerManager>().CreateViewTimer(0.2f, false, new TimerDelegate(this.OnTimerCallback), null);
		...
		public void HideObjectivesUI()
		{
			...
			if (base.Root != null && this.CurrentPlanet.PlanetGameObject != null)
			{
				...
				{
					Service.Get<ViewTimerManager>().KillViewTimer(this.animDelayTimerId);
				}
		...
		{
			Service.Get<GalaxyViewController>().StartPlanetTransition();
		}
		...
		public void GoToMainSelectScreen()
		{
			...
			this.buttonPlanetAction.Enabled = Service.Get<CurrentPlayer>().IsCurrentPlanet(this.CurrentPlanet.VO);
		}
		...
		{
			Planet planet = Service.Get<GalaxyViewController>().TransitionToPrevPlanet();
			Service.Get<EventManager>().SendEvent(EventId.GalaxyTransitionToPreviousPlanet, null);
			if (this.largeObjectivesShowing)
			{
		...
		{
			Planet planet = Service.Get<GalaxyViewController>().TransitionToNextPlanet();
			Service.Get<EventManager>().SendEvent(EventId.GalaxyTransitionToNextPlanet, null);
			if (this.largeObjectivesShowing)
			{
				this.objectivesView.SendObjectiveDetailsClickedBiLog(planet.VO);
			}
