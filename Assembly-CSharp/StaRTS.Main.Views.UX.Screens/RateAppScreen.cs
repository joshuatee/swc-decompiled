...
namespace StaRTS.Main.Views.UX.Screens
{
	public class RateAppScreen : ClosableScreen
	{
		...
		public RateAppScreen() : base("gui_rateapp")
		{
			...
			Service.Get<AssetManager>().Load(ref this.obiWanHandle, "gui_obiwan", new AssetSuccessDelegate(this.OnObiWanLoadSuccess), new AssetFailureDelegate(this.OnObiWanLoadFail), null);
		}
		...
		private void OnObiWanLoadSuccess(object asset, object cookie)
		{
			...
			this.obiWanObject = Service.Get<AssetManager>().CloneGameObject(asset as GameObject);
		...
		public override void Close(object modalResult)
		{
			...
			Service.Get<EventManager>().SendEvent(EventId.RateAppScreenClosed, null);
		}

		private void AwardLocalCrystals(DefaultResponse response, object cookie)
		{
			Service.Get<CurrentPlayer>().IsRateIncentivized = true;
			Service.Get<CurrentPlayer>().Inventory.ModifyCrystals(GameConstants.RATE_APP_INCENTIVE_CRYSTALS);
		}
		...
		public override void OnDestroyElement()
		{
			...
			{
				Service.Get<AssetManager>().Unload(this.obiWanHandle);
				this.obiWanHandle = AssetHandle.Invalid;
			}
