...
namespace StaRTS.Main.Views.UX.Screens
{
	public class ActivePerksInfoScreen : ClosableScreen, IViewClockTimeObserver
	{
		...
		private void AddPerkToGrid(ActivatedPerkData perkData)
		{
			...
			PerkVO perkVO = Service.Get<IDataController>().Get<PerkVO>(perkId);
			...
			while (i < num)
			{
				...
				PerkEffectVO perkEffectVO = Service.Get<IDataController>().Get<PerkEffectVO>(text);
				UXLabel uXLabel = (UXLabel)subElement5.CloneTemplateItem(text);
				uXLabel.Text = this.lang.Get(perkEffectVO.StatStringId, new object[0]) + Service.Get<PerkViewController>().GetFormattedValueBasedOnEffectType(perkEffectVO, null);
