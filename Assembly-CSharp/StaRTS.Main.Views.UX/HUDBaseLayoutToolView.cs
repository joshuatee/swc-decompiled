...
namespace StaRTS.Main.Views.UX
{
	public class HUDBaseLayoutToolView : IEventObserver
	{
		...
		{
			string title = Service.Get<Lang>().Get("WAR_BASE_PREP_END_TITLE", new object[0]);
			string message = Service.Get<Lang>().Get("WAR_BASE_PREP_END_TEXT", new object[0]);
			string primaryLabelText = Service.Get<Lang>().Get("WAR_BASE_PREP_END_CTA", new object[0]);
		...
		private void OnEnterBLTButtonClicked(UXButton button)
		{
			...
			{
				Service.Get<BILoggingController>().TrackGameAction("UI_edit_mode", "enter", null, null);
			}
			Service.Get<GameStateMachine>().SetState(new BaseLayoutToolState());
		}
		...
		private void OnCancelConfirmationPopupClosed(object result, bool selected)
		{
			if (result != null)
			{
				...
				if (selected)
				{
					Service.Get<SharedPlayerPrefs>().SetPref("SkipBLTCancel", "1");
				}
			}
		}
		...
		private void LogSaveLayoutButton()
		{
			...
			Service.Get<BILoggingController>().TrackGameAction(this.GetBaseEditingBILoggingContext(), action, message, null);
		}
		...
		private string GetBaseEditingBILoggingContext()
		{
			...
			if (Service.Get<GameStateMachine>().CurrentState is WarBaseEditorState)
			{
				result = Service.Get<SquadController>().WarManager.CurrentSquadWar.WarId;
			}
			return result;
		}
		...
		{
			return Service.Get<SquadController>().StateManager.GetCurrentSquad().SquadID;
		}
		...
		private void OnSaveConfirmationPopupClosed(object result, bool selected)
		{
			if (result != null)
			{
				Service.Get<BaseLayoutToolController>().SaveMap();
				this.LogSaveLayoutButton();
				this.ExitBaseLayoutTool(true);
				if (selected)
				{
					Service.Get<SharedPlayerPrefs>().SetPref("SkipBLTSave", "1");
				}
			}
		}
		...
		{
			Service.Get<BaseLayoutToolController>().StashAllBuildings();
		...
		private void UnstashBuilding(string buildingUID)
		{
			...
			Service.Get<UXController>().HUD.ShowContextButtons(Service.Get<BuildingController>().SelectedBuilding);
		}
		...
		public void RefreshStashedBuildingCount(string buildingUID)
		{
			...
			if (num > 0)
			{
				if (uXElement == null)
				{
					BuildingTypeVO vo = Service.Get<IDataController>().Get<BuildingTypeVO>(buildingUID);
		...
		{
			string pref = Service.Get<SharedPlayerPrefs>().GetPref<string>("BLTSeen");
			if (pref != "1")
			{
