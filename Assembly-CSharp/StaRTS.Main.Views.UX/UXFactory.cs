...
namespace StaRTS.Main.Views.UX
{
	public class UXFactory : UXElement, IEventObserver
	{
		...
		protected void Load(ref AssetHandle uxAssetHandle, string uxAssetName, UXFactoryLoadDelegate onSuccess, UXFactoryLoadDelegate onFailure, object cookie)
		{
			...
			this.isCachedScreen = (Service.IsSet<ScreenController>() && Service.Get<ScreenController>().LoadCachedScreen(ref uxAssetHandle, uxAssetName, new AssetSuccessDelegate(this.LoadSuccessNoClone), new AssetFailureDelegate(this.LoadFailure), cookie));
			if (!this.isCachedScreen)
			{
				Service.Get<AssetManager>().Load(ref uxAssetHandle, uxAssetName, new AssetSuccessDelegate(this.LoadSuccess), new AssetFailureDelegate(this.LoadFailure), null);
			}
		}

		protected void Unload(AssetHandle uxAssetHandle, string uxAssetName)
		{
			if (!this.isCachedScreen || !Service.Get<ScreenController>().UnloadCachedScreen(uxAssetName))
			{
				Service.Get<AssetManager>().Unload(uxAssetHandle);
			}
		}
		...
		private void FinishSetup(GameObject gameObject)
		{
			...
			{
				Service.Get<ScreenController>().AdjustDepths();
			}
			if (Service.IsSet<EventManager>())
			{
				Service.Get<EventManager>().SendEvent(EventId.UxFactoryLoadSuccess, this);
			}
		}
		...
		private UXLabel CreateLabel(GameObject gameObject, UILabel nguiLabel)
		{
			...
			{
				text = Service.Get<Lang>().Get(text, new object[0]);
			}
			uXLabel.Text = text;
