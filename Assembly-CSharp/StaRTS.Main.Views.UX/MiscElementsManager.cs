...
namespace StaRTS.Main.Views.UX
{
	public class MiscElementsManager : UXFactory, IEventObserver, IUserInputObserver, IViewFrameTimeObserver
	{
		...
		public MiscElementsManager(AssetsCompleteDelegate onCompleteCallback, object onCompleteCookie) : base(Service.Get<CameraManager>().UXCamera)
		{
			...
			this.mainCamera = Service.Get<CameraManager>().MainCamera;
		...
		public override void OnDestroyElement()
		{
			...
			Service.Get<EventManager>().UnregisterObserver(this, EventId.SquadAdvancementTabSelected);
		...
		private void LoadSuccess(object cookie)
		{
			...
			this.objectiveToast.Root.transform.parent = Service.Get<UXController>().WorldAnchor.Root.transform;
			...
			Service.Get<EventManager>().RegisterObserver(this, EventId.ShowObjectiveToast, EventPriority.Default);
			...
			this.fingerAnimation.Parent = Service.Get<UXController>().WorldAnchor;
		...
		private void SetupPlayerInstructionElements(UXLabel label, UXElement panel)
		{
			...
			Service.Get<CameraManager>().UXCamera.AttachToMainAnchor(panel.Root);
		}
		...
		{
			if (Service.Get<GameStateMachine>().CurrentState is NeighborVisitState)
			{
				this.HideEventsTickerView();
				return;
			}
		...
		{
			bool result = false;
			string pref = Service.Get<SharedPlayerPrefs>().GetPref<string>(GameConstants.PLANET_RELOCATED_TUTORIAL_ID);
		...
		private void ShowGalaxyTournamentStatus()
		{
			...
			{
				this.ConflictStatusTitleTimerId = Service.Get<ViewTimerManager>().CreateViewTimer(0.5f, false, new TimerDelegate(this.ShowGalaxyTournamentStatusCallBack), null);
			}
		}
		...
		private void ShowGalaxyTournamentStatusCallBack(uint timerId, object cookie)
		{
			...
			if (this.tournamentDisplayActive)
			{
				...
				this.ConflictStatusShowTimerId = Service.Get<ViewTimerManager>().CreateViewTimer(0.5f, false, new TimerDelegate(this.AnimateGalaxyTournamentStatus), null);
			}
		}
		...
		{
			Service.Get<ViewTimerManager>().KillViewTimer(this.ConflictStatusTitleTimerId);
			Service.Get<ViewTimerManager>().KillViewTimer(this.ConflictStatusShowTimerId);
			this.ConflictStatusTitleTimerId = 0u;
		...
		{
			TournamentProgress tournamentProgress = Service.Get<CurrentPlayer>().TournamentProgress;
			...
			this.eventTickerView.SetTitleText(Service.Get<Lang>().Get("CONFLICT_STATUS_TITLE", new object[0]));
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (tournamentVO != null)
				{
					...
					if (tournament != null && tournament.CurrentRank != null && !string.IsNullOrEmpty(tournament.CurrentRank.TierUid))
					{
						...
						{
							TournamentTierVO tournamentTierVO = Service.Get<IDataController>().Get<TournamentTierVO>(tournament.CurrentRank.TierUid);
		...
		private void SetupPlanetGalaxyUI()
		{
			...
			this.planetFrontGroup.transform.parent = Service.Get<UXController>().WorldAnchor.Root.transform;
		...
		private void TryWaitForRaidTime()
		{
			...
			if (num > 0)
			{
				this.raidTimerId = Service.Get<ViewTimerManager>().CreateViewTimer((float)num, true, new TimerDelegate(this.OnShowRaidTimer), null);
			}
		}
		...
		private void CancelWaitForRaidTime()
		{
			...
			{
				Service.Get<ViewTimerManager>().KillViewTimer(this.raidTimerId);
				this.raidTimerId = 0u;
			}
		}
		...
		private void UpdateRaidTimer()
		{
			...
			string message = Service.Get<Lang>().Get(this.raidTimerText, new object[]
			{
				text
			});
			EventTickerObject tickerObject = this.eventTickerView.GetTickerObject(0);
		...
		private void UpdateSquadWarTimer()
		{
			...
			string text2 = Service.Get<Lang>().Get("WAR_BASE_PREP_TIME_REMAINING", new object[]
			{
				text
			});
		...
		private void UpdateCurrencyTrayContents(UXElement screenRoot)
		{
			...
			for (int i = 0; i < this.currencyTrayTable.Count; i++)
			{
				...
				if (flag)
				{
					this.currencyTrayTable.GetSubElement<UXLabel>(text, "CurrencyLabel").Text = Service.Get<Lang>().ThousandsSeparated(num);
				}
			}
		...
		private void SetupConfirmGroup()
		{
			...
			this.confirmGroup.transform.parent = Service.Get<UXController>().WorldAnchor.Root.transform;
		...
		public void ShowTroopCounter(UXElement troopButton, int currentCount, int maxCount)
		{
			...
			this.troopCounterLabel.Text = Service.Get<Lang>().Get("FUE_TRAIN_TROOPS", new object[]
			{
				currentCount,
				maxCount
			});
		...
		public void HighlightButton(UXElement button, string arrowPosition)
		{
			...
			{
				UXSprite uXSprite;
				if (Service.Get<UXController>().HUD.IsElementProvidedTroop(button))
				{
					element.Visible = false;
			...
			{
				Service.Get<ViewTimerManager>().KillViewTimer(this.hideHighlightTimerId);
				this.hideHighlightTimerId = 0u;
				Service.Get<EventManager>().SendEvent(EventId.ButtonHighlightHidden, null);
			}
			...
			Service.Get<EventManager>().SendEvent(EventId.ButtonHighlightActivated, null);
		}
		...
		private void UpdateFingerPosition(float dt)
		{
			...
			Vector3 localPosition = Service.Get<CameraManager>().MainCamera.WorldPositionToScreenPoint(worldPoint);
		...
		public void ShowHudTooltip(UXElement element)
		{
			if (!this.wasHudToolTipVisible)
			{
				...
				this.hudTooltipLabel.Text = Service.Get<Lang>().Get("Tooltip_" + element.Root.name, new object[0]);
			}
		}
		...
		public void ShowSquadLevelTooltip(UXElement element)
		{
			if (!this.wasSquadLevelToolTipVisible)
			{
				...
				this.squadLevelTooltipLabel.Text = Service.Get<Lang>().Get("PERK_SQUAD_LEVEL_TOOLTIP", new object[0]);
		...
		public void ShowHudFactionIconTooltip(UXElement element)
		{
			if (!this.wasHudFactionToolTipVisible)
			{
				...
				Lang lang = Service.Get<Lang>();
				int totalVictoryRatingToNextLevel = Service.Get<FactionIconUpgradeController>().GetTotalVictoryRatingToNextLevel();
				int currentPlayerVictoryRating = Service.Get<FactionIconUpgradeController>().GetCurrentPlayerVictoryRating();
		...
		public void ShowHudBuffToolTip(UXElement element, List<WarBuffVO> buffs, bool rightSide)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				this.hudBuffToolTipTable.GetSubElement<UXLabel>(itemUid, "LabelTitleBuffsInfo").Text = Service.Get<Lang>().Get(buffs[i].BuffStringTitle, new object[0]);
				this.hudBuffToolTipTable.GetSubElement<UXLabel>(itemUid, "LabelBodyBuffsInfo").Text = Service.Get<Lang>().Get(buffs[i].BuffStringDesc, new object[0]);
		...
		public void ShowObjectiveToast(ObjectiveProgress objProgress)
		{
			if (this.objectiveToast != null && !this.isObjectiveToastVisible)
			{
				...
				CrateVO crateVO = Service.Get<IDataController>().Get<CrateVO>(objProgress.VO.CrateRewardUid);
		...
		public void ShowToast(string toast, string status, string crateName)
		{
			...
			this.objectiveDisplayTimerId = Service.Get<ViewTimerManager>().CreateViewTimer(3f, false, new TimerDelegate(this.HideToastByTimerTrigger), null);
		}
		...
		private void HideObjectiveToast()
		{
			...
			{
				Service.Get<ViewTimerManager>().KillViewTimer(this.objectiveDisplayTimerId);
			...
			if (this.objectiveToast != null && this.isObjectiveToastVisible)
			{
				...
				this.ResetObjectiveHideTimer();
				this.objectiveDisplayHideTimerId = Service.Get<ViewTimerManager>().CreateViewTimer(0.5f, false, new TimerDelegate(this.DisableToast), null);
			}
		}
		...
		private void ResetObjectiveHideTimer()
		{
			...
			{
				Service.Get<ViewTimerManager>().KillViewTimer(this.objectiveDisplayHideTimerId);
				this.objectiveDisplayHideTimerId = 0u;
			}
		}
		...
		public void EnableConfirmGroupAcceptButton(bool enable)
		{
			...
			if (enable)
			{
				Service.Get<UserInputInhibitor>().AddToAllow(this.acceptButton);
			}
		}
		...
		private void UpdatePlanetUIPosition()
		{
			...
			if (this.attachedPlanetFrontObject != null)
			{
				...
				if (this.frontPlanetData != null)
				{
					this.planetFrontStatsCount.Text = Service.Get<Lang>().ThousandsSeparated(this.frontPlanetData.ThrashingPopulation);
				}
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					if (Service.Get<GalaxyViewController>().ShouldAnimateCurrent(planet) && planet != this.frontPlanetData && planet.TournamentState != TimedEventState.Live)
					{
		...
		public void ShowPlanetBackUI(Planet planet)
		{
			...
			if (planetBackUIElement != null)
			{
				...
				this.GetChildElementLabel(planetBackUIElement, labelName).Text = Service.Get<Lang>().Get("PLANETS_CONFLICT_BACK", new object[]
				{
		...
		{
			this.planetTopRightAnchor.Parent = Service.Get<UXController>().WorldAnchor;
		...
		private void AttachFrontUIInternal(Planet planet)
		{
			...
			this.planetFrontStatsCount.Text = Service.Get<Lang>().ThousandsSeparated(planet.ThrashingPopulation);
			if (this.attachedPlanetFrontObject != null)
			{
			...
			this.planetSquadCount.Text = Service.Get<Lang>().ThousandsSeparated(planet.NumSquadmatesOnPlanet);
			this.ClearFriendsPicGrid();
		...
		public void ShowPlanetFrontUI(Planet planet)
		{
			if (this.frontPlanetUIState != GalaxyFrontPlanetUIState.Shown)
			{
				...
				Service.Get<EventManager>().SendEvent(EventId.GalaxyUIPlanetFocus, null);
			}
		}
		...
		public void ClearFriendsPicGrid()
		{
			...
			{
				Service.Get<ISocialDataController>().DestroyFriendPicture(this.planetFriendPicTextures[i]);
			}
		...
		public void OnViewFrameTime(float dt)
		{
			...
			{
				ObjectiveController objectiveController = Service.Get<ObjectiveController>();
