...
namespace Net.RichardLord.Ash.Core
{
	public class NodeList<TNode> : INodeList where TNode : Node<TNode>, new()
	{
		public event Action<TNode> NodeAdded
		{
			...
			add
			{
				this.NodeAdded = (Action<TNode>)Delegate.Combine(this.NodeAdded, value);
			}
			[MethodImpl(MethodImplOptions.Synchronized)]
			remove
			{
				this.NodeAdded = (Action<TNode>)Delegate.Remove(this.NodeAdded, value);
			}
		}
		...
		public event Action<TNode> NodeRemoved
		{
			...
			add
			{
				this.NodeRemoved = (Action<TNode>)Delegate.Combine(this.NodeRemoved, value);
			}
			[MethodImpl(MethodImplOptions.Synchronized)]
			remove
			{
				this.NodeRemoved = (Action<TNode>)Delegate.Remove(this.NodeRemoved, value);
			}
		}
