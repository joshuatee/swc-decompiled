	...
	public class OwnUnitCondition : AbstractCondition, IEventObserver
	{
		...
		public override void GetProgress(out int current, out int total)
		{
			...
			if (!this.any)
			{
				...
				{
					TroopTypeVO troopTypeVO = Service.Get<IDataController>().Get<TroopTypeVO>(this.unitId);
					list = Service.Get<TroopUpgradeCatalog>().GetUpgradeGroupLevels(troopTypeVO.UpgradeGroup);
					break;
				}
				case ConditionMatchType.Id:
					list = Service.Get<TroopUpgradeCatalog>().GetUpgradeGroupLevels(this.unitId);
					break;
