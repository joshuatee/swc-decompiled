...
namespace StaRTS.Main.Views.UX.Screens.Squads
{
	public class SquadScreenAdvancementView : AbstractSquadScreenViewModule, IEventObserver
	{
		...
		{
			if (this.activeAdvancementTab != null && !this.activeAdvancementTab.ShouldBlockInput() && Service.Get<PerkManager>().HasPlayerSeenPerkTutorial())
			{
				Service.Get<UXController>().MiscElementsManager.ShowSquadLevelTooltip(button);
			}
		}
