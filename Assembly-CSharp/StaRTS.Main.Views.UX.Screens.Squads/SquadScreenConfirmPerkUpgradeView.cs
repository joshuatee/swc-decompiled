...
namespace StaRTS.Main.Views.UX.Screens.Squads
{
	public class SquadScreenConfirmPerkUpgradeView
	{
		...
		private void OnInvestClicked(UXButton btn)
		{
			...
			Service.Get<PerkManager>().InvestInPerk(this.nextInvestAmt, this.perkToInvestIn.Uid, perks.Available, squadLevelUIDFromSquad);
			Service.Get<EventManager>().SendEvent(EventId.PerkInvested, null);
			this.nextInvestAmt = 0;
			this.UpdateInvestUIBasedOnNewAmt();
		}
		...
		private void UpdateInternalInvestCountFromServerData()
		{
			...
			int perkInvestedProgress = Service.Get<PerkManager>().GetPerkInvestedProgress(this.perkToInvestIn, inProgress);
