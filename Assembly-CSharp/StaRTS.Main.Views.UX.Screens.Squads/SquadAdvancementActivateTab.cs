...
namespace StaRTS.Main.Views.UX.Screens.Squads
{
	public class SquadAdvancementActivateTab : SquadAdvancementBaseTab, IEventObserver, IViewClockTimeObserver
	{
		...
		{
			Dictionary<string, uint> playerPerkGroupCooldowns = Service.Get<PerkManager>().GetPlayerPerkGroupCooldowns();
			IDataController dataController = Service.Get<IDataController>();
		...
		private void OnCooldownButtonClicked(UXButton button)
		{
			...
			string perkNameForGroup = Service.Get<PerkViewController>().GetPerkNameForGroup(perkVO.PerkGroup);
			...
			Service.Get<PerkManager>().ConfirmPurchaseCooldownSkip(uid, perkNameForGroup, LangUtils.FormatTime((long)perkCooldownTimeLeft), crystalCost);
		}
		...
		private void OnRemoveButtonClicked(UXButton button)
		{
			...
			Service.Get<PerkViewController>().ShowCancelPerkAlert(text, perkVO.PerkGroup);
		}
