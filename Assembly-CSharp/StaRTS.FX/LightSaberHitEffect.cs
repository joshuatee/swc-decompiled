	...
	public class LightSaberHitEffect
	{
		...
		public LightSaberHitEffect(int radius, int gridX, int gridZ, Transform heroTrans, string centerFx, FactionType faction)
		{
			...
			if (heroTrans != null)
			{
				...
				{
					effectsTypeVO = dataController.Get<EffectsTypeVO>("effect199");
				}
				else
				{
					effectsTypeVO = dataController.Get<EffectsTypeVO>("effect197");
				}
				if (effectsTypeVO != null)
				{
					text = effectsTypeVO.AssetName;
				}
				effectsTypeVO = dataController.Get<EffectsTypeVO>("effect198");
				if (effectsTypeVO != null)
				{
				...
				if (centerFx != null)
				{
					Service.Get<AssetManager>().Load(ref this.circularHitFxHandle, centerFx, new AssetSuccessDelegate(this.OnLoadCircularFx), null, null);
				}
		...
		public void StopFxAndDestroy()
		{
			...
			Service.Get<ViewTimerManager>().CreateViewTimer(1f, false, new TimerDelegate(this.OnDestroyHitFxTimer), null);
			Service.Get<ViewTimerManager>().CreateViewTimer(3f, false, new TimerDelegate(this.OnDestroySwirlFxTimer), null);
		}
		...
		private void OnLoadHitFx(object asset, object cookie)
		{
			...
			Service.Get<AssetManager>().Load(ref this.adjHitFxHandle, assetName, new AssetSuccessDelegate(this.OnLoadAdjHitFx), null, null);
		}
		...
		private void OnLoadAdjHitFx(object asset, object cookie)
		{
			...
			while (i < count)
			{
				if (this.hitFx != null && this.targetLocators[i] != null)
				{
					...
					Service.Get<ViewTimerManager>().CreateViewTimer(num * (float)i, false, new TimerDelegate(this.OnPlayMainFxTimer), dictionary);
				}
				if (this.adjHitFx != null)
				{
					...
					Service.Get<ViewTimerManager>().CreateViewTimer(num * (float)i, false, new TimerDelegate(this.OnPlayFxTimer), cookie2);
					Service.Get<ViewTimerManager>().CreateViewTimer(num * (float)i, false, new TimerDelegate(this.OnPlayFxTimer), cookie3);
				}
				i++;
			}
		}
		...
		private void OnDestroySwirlFxTimer(uint id, object cookie)
		{
			...
			{
				Service.Get<AssetManager>().Unload(this.circularHitFxHandle);
		...
		private void OnDestroyHitFxTimer(uint id, object cookie)
		{
			...
			{
				Service.Get<AssetManager>().Unload(this.hitFxHandle);
				this.hitFxHandle = AssetHandle.Invalid;
			}
			if (this.adjHitFxHandle != AssetHandle.Invalid)
			{
				Service.Get<AssetManager>().Unload(this.adjHitFxHandle);
