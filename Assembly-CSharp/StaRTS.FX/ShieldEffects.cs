...
namespace StaRTS.FX
{
	public class ShieldEffects
	{
		...
		{
			Service.Set<ShieldEffects>(this);
		...
		private ShieldBuildingInfo LoadEffectsForBuilding(Entity building)
		{
			...
			Service.Get<AssetManager>().MultiLoad(list4, list2, new AssetSuccessDelegate(this.OnEffectLoaded), null, list3, new AssetsCompleteDelegate(this.OnEffectsComplete), shieldBuildingInfo);
		...
		public void ApplyHitEffect(Entity building, Vector3 worldPosition)
		{
			...
			Service.Get<ViewTimerManager>().CreateViewTimer(1.2f, false, new TimerDelegate(this.CleanUpHitEffect), impactCookie);
		}
		...
		private void PlayEffect(Entity building, bool idle, float delay)
		{
			if (delay > 0f)
			{
				...
				{
					uint value = Service.Get<ViewTimerManager>().CreateViewTimer(delay, false, new TimerDelegate(this.PlayDelayedEffect), building);
			...
			if (!idle)
			{
				Service.Get<EventManager>().SendEvent(EventId.ShieldStarted, building);
			}
		}
		...
		public void PlayAllEffects(bool idle)
		{
			...
			{
				if (!Service.Get<BaseLayoutToolController>().IsBuildingStashed(shieldGeneratorNode.Entity))
				{
		...
		public void CleanupShield(Entity building)
		{
			...
			if (this.delayTimers.ContainsKey(building))
			{
				...
				Service.Get<ViewTimerManager>().KillViewTimer(id);
			...
			while (i < count)
			{
				Service.Get<AssetManager>().Unload(shieldBuildingInfo.AssetHandles[i]);
				i++;
			}
		...
		public void Cleanup()
		{
			...
			{
				Service.Get<ViewTimerManager>().KillViewTimer(current);
			}
			...
			foreach (ShieldBuildingInfo current2 in this.buildings.Values)
			{
				...
				while (i < count)
				{
					Service.Get<AssetManager>().Unload(current2.AssetHandles[i]);
					i++;
				}
