...
namespace StaRTS.Main.Views.Animations
{
	public class TransitionVisuals : UXFactory
	{
		...
		public TransitionVisuals(PlanetVO planet, TransitionVisualsLoadedDelegate onLoaded, object cookie, bool onlyLoadUI) : base(Service.Get<CameraManager>().UXSceneCamera)
		{
			if (planet == null)
			{
				planet = Service.Get<IDataController>().Get<PlanetVO>("planet1");
			}
			this.planet = planet;
		...
		private void UILoaded(object cookie)
		{
			...
			{
				optionalElement.Text = Service.Get<Lang>().Get(this.planet.LoadingScreenText, new object[0]);
			}
		...
		private void AssembleLoadingScene()
		{
			...
			{
				Camera camera = Service.Get<CameraManager>().UXSceneCamera.Camera;
				if (this.planetGameObject != null)
				{
					...
					Service.Get<GalaxyPlanetController>().SetPlanetLighting(this.planetGameObject, this.planetMaterial, camera);
				}
				if (this.planetGlowGameObject != null)
				{
		...
		{
			Service.Get<CameraManager>().UXSceneCamera.Camera.enabled = true;
