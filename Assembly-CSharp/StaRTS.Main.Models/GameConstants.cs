...
namespace StaRTS.Main.Models
{
	public static class GameConstants
	{
		...
		{
			if (!GameConstants.MAKER_VIDEO_ENABLED)
			{
				return false;
			}
			if (!Service.IsSet<Lang>())
			{
				Service.Get<Logger>().Error("Language is not initialized!");
				return false;
			}
			if (GameConstants.MakerVideoLocales == null)
			{
				Service.Get<Logger>().Error("MakerVideoLocales is not initialized!");
				return false;
			}
			if (GameConstants.MakerVideoLocales.Length == 0)
			{
				Service.Get<Logger>().Error("No MakerVideoLocales set!");
				return false;
			}
			return Array.IndexOf<string>(GameConstants.MakerVideoLocales, Service.Get<Lang>().Locale) != -1;
		}
		...
		private static void InitWarBaseLevelMapping()
		{
			...
			{
				Service.Get<Logger>().Error("GameConstants.WAR_BUFF_BASE_HQ_LEVEL_MAP is null or empty");
				return;
			}
		...
		private static void InitRelocationCrystalsAndStarCost()
		{
			...
			{
				Service.Get<Logger>().Error("GameConstants.CRYSTALS_PER_RELOCATION_STAR is null or empty");
				return;
			}
			...
			{
				Service.Get<Logger>().Error("RelocationCrystalsAndStarCost Invalid Lengths");
			...
			for (int i = 0; i < num; i++)
			{
				...
				{
					Service.Get<Logger>().Error("RelocationCrystalsAndStarCost Parse Error");
					GameConstants.CrystalsPerRelocationStar = new int[0];
					GameConstants.StarsPerRelocation = new int[0];
					return;
				}
		...
		private static void InitMakerVideo()
		{
			...
			{
				Service.Get<Logger>().Error("GameConstants.MAKER_VIDEO_LOCALES is null or empty");
				return;
			}
